...
DocumentManager.onReceiveComplete = function(document) {
	...
	editorPane.getCaret().pos = DocumentManager.getCurrentSection().children[0].children[0].id;
...
DocumentManager.isEditable = function() {
	...
	if (this.instance.currentDocument === null) return false;
	var result = this.instance.currentDocument.isEditable();
	if (!result) return false;

	// 読み上げモードオンの時は -1 を返します
	if (ReadManager.instance.readingMode) return -1;

	// readonly であれば、編集不可とします
	if (DocumentManager.isReadOnly()) return 0;
 ...
 * 文書が読み込み専用か否か取得します。
 */
DocumentManager.isReadOnly = function() {
...
DocumentManager._insertDocument = function(document, insertLast) {
	...
	// ノードは別のノードに挿入すると元の箇所から削除されるため、
	// 以下のようなコードになります。
	while (newSectionList.children.length > 0) {
		sectionList.insertBefore(newSectionList.children[0], at);
	}
