...
//	var clipText = inputbox.textContent;
	...
	// span 要素を空にします
	// ★以下の処理を変更する場合は、line 67 の処理も見直してください
	inputbox.innerHTML = '';
...
ClipboardManager.createFromImg = function(img) {
	// ※リンク画像は貼り付けられません
	if (img.src.substr(0, 11).toLowerCase() !== 'data:image/') {
		throw 'Can not paste linked image.';
	}
...
ClipboardManager.createFromPlainText = function(text) {
	...
	for (var i = 0; i < text.length; i++) {
		var entity = null;
		if (text.substr(i, 1) === ' ') entity = '&nbsp;';
		list.push(CharacterElement.createNew(text.substr(i, 1), entity));
	};
...
ClipboardManager.createFromPlainTextWithBr = function(text) {
	...
	for (var cidx = 0; cidx < text.length; cidx++) {
		...
		if (char !== '\n') {
			...
			var entity = null;
			if (char === ' ') entity = '&nbsp;';
			workingList.push(CharacterElement.createNew(char, entity));
		} else {
