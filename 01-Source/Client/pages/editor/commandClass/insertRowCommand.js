 ...
 */
var InsertRowCommand = function(caretId, isBefore) {
	// ドキュメントの参照を取得します。
	// 範囲選択リストを取得します。
	...
	this.prepData = TableEditor.prepareInsertRow(this.caretId);
};

/**
 * Command基底クラスを継承します。
 */
InsertRowCommand.prototype = new BaseCommand();

/**
 * execute()をオーバーライドします。
 */
InsertRowCommand.prototype.execute = function() {
 ...
 * undo()をオーバーライドします。
 */
InsertRowCommand.prototype.undo = function() {
	TableEditor.removeRow(this.prepData.newCellList[0]);
 ...
 * redo()をオーバーライドします。
 */
InsertRowCommand.prototype.redo = function() {
	...
	TableEditor.insertRow(baseNodeId, this.isBefore, newCellList, tableNode);
 ...
 */
InsertRowCommand.prototype.getMemCost = function() {
	return 1;
};
