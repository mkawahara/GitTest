/**
 * 閉じ括弧を使用する連立方程式などを表現します。
 */

function CloseBracketElement() {
	// 何もしません
};


//基本クラスの継承
CloseBracketElement.prototype = Object.create(BracketBaseClass.prototype);


/**
 * 新しいCloseBracketElementのインスタンスを作成します。
 * @param rows	行数
 * @param cols	列数
 */
CloseBracketElement.createNew = function(bracket, xmlType) {
	// 不適切な括弧が指定されていた場合、丸括弧に強制します
	if (bracket.length < 1) bracket = ')';
    if (xmlType == CIO_XML_TYPE.text) xmlType = CIO_XML_TYPE.math;

	// 新しいノードインスタンスを作成します
	var node = document.createElement('cclose'); // タグ名は全て小文字
	...
	node.setAttribute('closebracket', bracket);
...
/**
 * 指定ノードのプロトタイプに CloseBracketElement を設定します
 */
CloseBracketElement.doop = function(node) {
	DataClass.insertPrototype(node, CloseBracketElement);
};
 ...
 */
CloseBracketElement.copyFrom = function(src) {
 ...
 */
CloseBracketElement.prototype.toHtml = function(caretId) {
	...
	// 開始文字列の作成
	startTag += '<mrow id="' + this.id + '" class="' + spkStr + '" style="border: solid 0px white;"><mtable>';
	endTag = '</mtable><mrow><mo>' + closebracket + '</mo></mrow></mrow>' + endTag;
 ...
 */
CloseBracketElement.prototype.compareWith = function(node, checkNext) {
	// ノード名とテキストを比較します
	if (node.nodeName !== 'CCLOSE') return null;
