...
/* ==                      株式会社 知能情報システム                      == */
/* ========================================================================= */
/* -    ファイル名     ： DC_TopElement.js                                   */
/* -                                                                         */
/* -    概      要     ： 上に文字・線を有する要素用データクラス             */
/* -                                                                         */
/* -    依      存     ： DC_enum.js, utility.js, その他親クラスファイル     */
/* -                                                                         */
/* -    備      考     ： 上に文字・線を有する要素用データクラスです         */
/* -                                                                         */
...
// TopElement.prototype = new GroupElement(); // 親クラスが GroupElementの例
// もしくは、LayoutBaseClass.prototypeを本オブジェクトのプロトタイプに設定します。
TopElement.prototype = Object.create(LayoutBaseClass.prototype);



// [static] ------------- クラス用の xmlノード DOM を用意し、文書内で一意なidを与えます。
TopElement.createNew = function(base, xmlType) {
	...
	var domObj = document.createElement('top'); // xmlタグ名は全て小文字です。
...
TopElement.doop = function(domObj) {
	// 返値 dom [obj]    : DOM
	// 返値なし
	DataClass.insertPrototype(domObj, TopElement); // utility.js 依存部分です。第二引数は、クラス名のみです。
};



// [static] ------------- 既存オブジェクトをコピーして新しいオブジェクトを作成します。
TopElement.copyFrom = function(src) {
 ...
 * @returns
 */
TopElement.prototype.getDefaultCursorPos = function() {
...
TopElement.prototype.toHtml = function(caretId) {
	...
	// mmultiscriptsタグを追加します。
	startTag += '<mmultiscripts id="' + this.id + '" class="' + spkStr + '" style="border: solid 0px white;">';
	endTag    = '</mmultiscripts>' + endTag;

	// moverタグを追加します。
	var children = this.children;   // 子要素配列への参照
	DataClass.bindDataClassMethods(children[0]);
	startTag += '<mover>' + children[0].toHtml(caretId) + '<mo>' + this.getAttribute('bs') + '</mo></mover>';
...
*/
TopElement.prototype.compareWith = function(node, checkNext) {
	// ノード名とテキストを比較します
	if (node.nodeName !== 'TOP') return null;
 ...
 */
TopElement.prototype.shiftRight = function() {
...
/**
 * ベース・添え字要素間での左移動時の次のカーソル位置を返します。
 */
TopElement.prototype.shiftLeftFromChild = function(childId) {
...
/**
 * ベース・添え字要素間での右移動時の次のカーソル位置を返します。
 */
TopElement.prototype.shiftRightFromChild = function(childId) {
	// 常に弟要素に移動します
 ...
 */
TopElement.prototype.shiftUpFromChild = function(childId) {
 ...
 */
TopElement.prototype.shiftDownFromChild = function(childId) {
 ...
 */
TopElement.prototype.shiftByEnterFromChild = function(childId) {
	// 下添字にカーソルがある場合、上添字のカーソル位置を取得します。
	if (this.children[1].id === childId) {
		var nextNode = this.children[2];
		DataClass.bindDataClassMethods(nextNode);
 ...
 */
TopElement.prototype.shiftHomeFromChild = function() {
	var parent = this.parentElement;
	DataClass.bindDataClassMethods(parent);
 ...
 * 親グループの同名メソッドを呼び出し、その戻り値を返します。
 */
TopElement.prototype.shiftEndFromChild = function() {
	var parent = this.parentElement;
	DataClass.bindDataClassMethods(parent);
 ...
 * 上添字の要素を取得します。
 */
TopElement.prototype.shiftUpFromNext = function() {
 ...
 * 下添字の要素を取得します。
 */
TopElement.prototype.shiftDownFromNext = function() {
 ...
 */
TopElement.prototype.shiftLastFromNext = function() {
