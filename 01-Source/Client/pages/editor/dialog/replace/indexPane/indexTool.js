...
//    // 先頭へカーソル移動　★暫定★　移動先がないとカーソル表示はできない、本当はカーソル移動もRendererでやるべき？
//      var section = DocumentManager.getCurrentSection();
//      var firstPara = section.firstChild;
//      if (firstPara) {
//          var firstNode = firstPara.firstChild;
//          if (firstNode) {
//              var caret = ViewManager.getEditorPane().getCaret();
//                EditorPaneClass.finalizeMoveCaret(null, firstNode.id);
//          }
//      }
...
IndexToolClass.keyOpeSectionItem = function(event) {
	...
	// ---- キー判定による処理分け
	if (event.shiftKey || event.ctrlKey) {                        // ---- Shift キーや Ctrl キーが押下されていれば
		if (event.ctrlKey) {                                              // ---- Ctrl キーなら
...
IndexToolClass.ReadySectionTitle = function() {
	...
	if (index !== null) $('#ST_SectionTitleBar').val(sectionInfoList[index].title);
};
...
//		localCopy.attr('id', IDT_SECTION_ID_HEADER + sectionInfo['id']);  // セクションID
		...
		if (tempTitle == '' || tempTitle == null) tempTitle = '< 段落 ' + (i + 1) + '>';
		localCopy.children( 'div:last' ).children( 'span' ).text( tempTitle );  // セクションタイトル
